...
face_cascade = cv2.CascadeClassifier("C:/Users/nvcmm/anaconda3/Lib/site-packages/cv2/data/haarcascade_frontalface_default.xml")
...
def create_contact_sheet(dictionary, name_present):
    number_of_rows = math.ceil(len(dictionary['thumbnail_list']) / 5)
    ...
    if name_present:
        draw_space.text((20, 20), 'Results found in file {}'.format(dictionary['filed_under']), font=font_thing)
        thumbnail_counter = 0
        for thumbnail in dictionary['thumbnail_list']:
        ...
        draw_space.text((20, 20), 'No results found in file {}'.format(dictionary['filed_under']), font=font_thing)
    return canvas
